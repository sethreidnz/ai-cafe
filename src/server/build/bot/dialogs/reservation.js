...
const registerReservationDialog = bot => {
  bot.dialog(DIALOG_NAMES.RESERVATION, [(session, args, next) => {
    session.dialogData.reservation = {};
    const reservationDate = args && builder.EntityRecognizer.findEntity(args.entities, ENTITY_NAMES.DATE_TIME);
    if (!reservationDate) {
